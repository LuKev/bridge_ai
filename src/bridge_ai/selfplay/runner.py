...
def run(config_path: str = "configs/default.yaml") -> ReplayBuffer:
    ...
    manifest_path = cfg_dict.get("storage", {}).get("manifest_path", "artifacts/manifest.json")
    ...
    _load_checkpoint_if_available(model, cfg.checkpoint)
    run_id = f"selfplay_{int(time.time() * 1_000_000)}"
    run_signature = compute_config_signature(
        config_path,
        extra={"seed": cfg.seed, "seed_sequence": cfg.seed_sequence, "num_episodes": cfg.num_episodes},
    )
    ...
    append_manifest_entry(
        ...
        outputs={"replay_path": str(out), "steps": len(buffer), "episodes": cfg.num_episodes},
        metrics={"episodes": cfg.num_episodes, "steps": len(buffer)},
        status="ok",
    )
    return buffer
